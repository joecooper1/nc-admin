...
class Students extends React.Component {
	...
	componentDidUpdate(prevProps, prevState) {
		if (this.state.just_graduated.length > prevState.just_graduated.length)
			axios.get('https://nc-student-tracker.herokuapp.com/api/students?sort_by=name').then((res) => {
	...
	selectStudent = (event) => {
		...
		axios.get(`https://nc-student-tracker.herokuapp.com/api/students/${name}`).then((res) => {
	...
	graduateStudents = (event) => {
		...
		this.state.graduating_students.map((student) =>
			axios
				.patch(`https://nc-student-tracker.herokuapp.com/api/students/${student}?progress=true`)
				.then((res) => {
					this.setState((currentState) => {
						return { just_graduated: [ ...currentState.just_graduated, res.data.student ] };
					});
	...
	addToGraduating = (event) => {
		...
		this.setState((currentState) => {
			if (check === true) {
				...
				return {
					graduating_students: currentState.graduating_students.filter((student) => student !== newStudent)
				};
		});
	};
}
